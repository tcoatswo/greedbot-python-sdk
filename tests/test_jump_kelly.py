...
def test_compute_jump_kelly_shape_and_bounds():
    ...
    assert len(f_stars) == 3
    for f in f_stars:
        assert 0.0 <= f <= 1.0


def test_merton_jump_kelly_sizer_class():
    ...
    assert "f_star" in res
    assert "target_risk_dollars" in res
    assert res["target_risk_dollars"] > 0
    assert res["target_risk_dollars"] <= 20000.0 * 0.25
